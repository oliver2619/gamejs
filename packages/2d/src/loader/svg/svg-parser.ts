...
import { PaintStyleWithGlobal, SvgParserContext } from "./svg-parser-context";
import { SvgElementData, SvgElementPort, SvgMaterializedElementData, SvgPort } from "./svg-port";
...
export class SvgParser {
    ...
    parseElement(input: SVGSVGElement, port: SvgPort) {
        this.parseChildren(input, port.root, SvgParserContext.newInstance(input, port));
    }
    ...
    private getElementData(el: SVGElement, context: SvgParserContext): SvgElementData {
    ...
    private getFillStyle(el: SVGElement, boundingBoxBottomLeft: ReadonlyVector2d, context: SvgParserContext): PaintStyle | undefined {
    ...
    private getMaterializedElementData(el: SVGGraphicsElement, context: SvgParserContext): SvgMaterializedElementData {
    ...
    private getPaintStyle(color: string, opacity: number, boundingBoxBottomLeft: ReadonlyVector2d, context: SvgParserContext): PaintStyle | undefined {
        if (color === 'none') {
            return undefined;
        }
    ...
    private getStrokeStyle(el: SVGElement, boundingBoxBottomLeft: ReadonlyVector2d, context: SvgParserContext): PaintStyle | undefined {
    ...
    private parseChildren(el: Element, obj: SvgElementPort, context: SvgParserContext) {
    ...
    private parseCircle(el: SVGCircleElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addCircle({
    ...
    private parseDefs(el: SVGDefsElement, context: SvgParserContext) {
        for (let i = 0; i < el.children.length; ++i) {
            const c = el.children.item(i);
            if (c != null) {
                switch (c.nodeName) {
    ...
    private _parseElement(el: Element, obj: SvgElementPort, context: SvgParserContext) {
        switch (el.nodeName) {
            case 'circle':
    ...
    private parseEllipse(el: SVGEllipseElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addEllipse({
            center: context.transform(el.cx.baseVal.value, el.cy.baseVal.value),
    ...
    private parseFilter(_1: SVGFilterElement, _2: SvgParserContext) {
        console.warn('<filter> elements are not supported yet');
        // TODO implement filter
    }

    private parseGroup(el: SVGGElement, obj: SvgElementPort, context: SvgParserContext) {
    ...
    private parseImage(el: SVGImageElement, obj: SvgElementPort, context: SvgParserContext) {
    ...
    private parseLine(el: SVGLineElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addLine({
    ...
    private parsePath(el: SVGPathElement, obj: SvgElementPort, context: SvgParserContext) {
    ...
    private parsePattern(el: SVGPatternElement, context: SvgParserContext) {
    ...
    private parsePolygon(el: SVGPolygonElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addPolygon({
            points: Array.from(el.points).map(it => context.transform(it.x, it.y))
        }, this.getMaterializedElementData(el, context));
    }

    private parsePolyline(el: SVGPolylineElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addPolyline({
            points: Array.from(el.points).map(it => context.transform(it.x, it.y))
        }, this.getMaterializedElementData(el, context));
    }

    private parseRect(el: SVGRectElement, obj: SvgElementPort, context: SvgParserContext) {
        obj.addRect({
            x: el.x.baseVal.value - context.viewboxLeft,
            y: context.viewboxBottom - el.y.baseVal.value - el.height.baseVal.value,
            width: el.width.baseVal.value,
    ...
    private parseSvg(el: SVGSVGElement, obj: SvgElementPort, context: SvgParserContext) {
        const g = obj.addGroup({
            coordSystem: this.getCoordSystemFromElement(el)
        }, this.getMaterializedElementData(el, context));
    ...
    private parseSymbol(_el: SVGSymbolElement, _context: SvgParserContext) {
        console.warn('<symbol> elements are not supported yet');
        // TODO
    }

    private parseText(_el: SVGTextElement, _obj: SvgElementPort, _context: SvgParserContext) {
        console.warn('<text> elements are not supported yet');
        // TODO
    }

    private parseUse(_el: SVGUseElement, _obj: SvgElementPort, _context: SvgParserContext) {
