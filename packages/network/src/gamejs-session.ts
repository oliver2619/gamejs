...
export class GamejsSession {
    ...
    private constructor(private readonly client: WebSocketClient) {
        client.onBlobMessage.subscribe(this, ev => this.onBlobMessage.next(ev));
        client.onClose.subscribe(this, () => this.onClose.next());
        client.onError.subscribe(this, ev => this.onError.next(ev));
        client.onStringMessage.subscribe(this, ev => this._onJsonMessage(JSON.parse(ev)));
    }
    ...
    private _onJsonMessage(message: GamejsSessionEvent) {
        switch (message.name) {
            case 'custom':
                this.onJsonMessage.next((message as GamejsEventCustom).data);
                break;
    ...
    private _onUpdateSession(ev: GamejsEventUpdateSessionData) {
        ...
        this.onUpdateSession.next(this);
    }

    private _onAddPlayer(player: string) {
        this._players.push(player);
        this.onAddPlayer.next(player);
    }
    ...
    private _onRemovePlayer(player: string) {
        ...
        if (found >= 0) {
            ...
            this.onRemovePlayer.next(player);
        }
    }
}