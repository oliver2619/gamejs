...
export interface BspTreeNode<P, O> {
    ...
    add(obj: O, position: ReadonlyVector3): BspTreeNode<P, O>;

    filterObjects(filter: (obj: O) => boolean): void;

    findNode(position: ReadonlyVector3): BspTreeNodeLeaf<P, O>;

    forEachBackToFront(position: ReadonlyVector3, lookDirection: ReadonlyVector3, visitor: BspTreeNodeVisitor<P, O>): void;

    forEachObject(callback: (obj: O) => any): void;

    forEachPolygon(position: ReadonlyVector3, callback: (polygon: P) => any): void;
...
export class BspTreeObject<O> {
    ...
    constructor(public obj: O, public position: Vector3) { }
}

export class BspTreeNodeLeaf<P, O> implements BspTreeNode<P, O> {

    private objects: Array<BspTreeObject<O>> = [];
    private lastSortVector: ReadonlyVector3 | undefined;
    ...
    findNode(_: Vector3): BspTreeNodeLeaf<P, O> {
        return this;
    }
    ...
    forEachPolygon(_: ReadonlyVector3, __: (polygon: P) => any): void { }

    isLeaf(): boolean { return true; }

    move(obj: O, newPos: ReadonlyVector3): void {
    ...
    private sortBackToFront(cameraLookDirection: ReadonlyVector3): void {
...
export class BspTree<P, O> {
    ...
    forEachPolygon(position: ReadonlyVector3, callback: (p: P) => any): void {
        this._root.forEachPolygon(position, callback);
    }

    move(obj: O, startNode: BspTreeNode<P, O>, endPosition: ReadonlyVector3): BspTreeNode<P, O> {
...
class BspTreePolygon<D> {
    ...
    constructor(public polygon: Polygon3, public data: D) { }
}
...
export class BspTreeBuilder<D> {
    ...
    addPolygon(polygon: Polygon3, data: D): BspTreeBuilder<D> {
        this.polygons.push(new BspTreePolygon(polygon, data));
        return this;
    }

    build<P, O>(conversion: (polygon: Polygon3, data: D) => P): BspTree<P, O> {
        return new BspTree(this.convert(this.polygons, conversion));
    }

    private convert<P, O>(polygons: Array<BspTreePolygon<D>>, conversion: (polygon: Polygon3, data: D) => P): BspTreeNode<P, O> {
